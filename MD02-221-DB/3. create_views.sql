...
CREATE VIEW search_by_part_of_name AS
SELECT * FROM menu_items WHERE name LIKE '%cake%';

-- Search by order/receipt number - select all matching menu items from that order/recipe
CREATE VIEW search_by_ticket_numeber AS
SELECT * FROM orders WHERE order_ticket_id=1;
...
CREATE VIEW sold_items_for_month AS
SELECT menu_items.name, orders.created_at, orders.paid
FROM orders INNER JOIN menu_items
ON orders.menu_item_id=menu_items.menu_item_id
...
FROM orders INNER JOIN menu_items ON orders.menu_item_id=menu_items.menu_item_id
WHERE orders.paid=1 AND MONTH(orders.created_at) = 8
GROUP BY menu_items.name;

-- 4. Add index on the appropriate places
CREATE INDEX id_index
ON orders (order_id, order_ticket_id );
